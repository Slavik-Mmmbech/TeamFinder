...
- Проекты имеют статус `open`/`closed`.
  ...
  - Только владелец проекта может завершить его.
- На странице проекта кнопка участия отображается только для авторизованных пользователей, не являющихся владельцем.
- `django.forms` ошибки используют встроенные шаблоны, поэтому важно, чтобы проект корректно загружал статические файлы и шаблоны.
- `.env` хранится локально и должен оставаться вне репозитория, а в `.gitignore` уже прописано его игнорирование.
