(function(){
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const addBtn = document.getElementById("add-user-skill-btn");
    const inputWrapper = document.getElementById("user-skill-input-wrapper");
    const input = document.getElementById("user-skill-input");
    const suggestions = document.getElementById("user-skill-suggestions");
    ...
    container.addEventListener("click", async (e) => {
      if (e.target.classList.contains("remove-skill-btn")) {
        ...
        const res = await fetch(`/users/skills/${skillId}/remove/`, { method: "POST", headers: { "X-CSRFToken": getCookie("csrftoken") } });
        if (res.ok) chip.remove();
      }
    });

    async function addSkillById(skillId) {
      const res = await fetch(`/users/skills/add/`, { method: "POST", headers: { "Content-Type": "application/json", "X-CSRFToken": getCookie("csrftoken") }, body: JSON.stringify({ skill_id: skillId }) });
      if (res.ok) {
    ...
    async function addSkillByName(name) {
      const res = await fetch(`/users/skills/add/`, { method: "POST", headers: { "Content-Type": "application/json", "X-CSRFToken": getCookie("csrftoken") }, body: JSON.stringify({ name }) });
      if (res.ok) {
        const skill = await res.json();
        appendChip(skill.id, skill.name);
      }
    ...
    function appendChip(id, name) {
      ...
      const empty = container.querySelector(".skill-empty"); if (empty) empty.remove();
    }

    function hideInput() { inputWrapper.classList.add("hidden"); suggestions.classList.add("hidden"); addBtn.classList.remove("hidden"); }
